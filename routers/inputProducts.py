...
# 데이터 모듈
from data import inputProducts as memo_data
from data import auth as auth_data

router = APIRouter(prefix="/inputProducts", tags=["products"])
...
async def create_memo(request: Request, memo: MemoRequest, db: Session = Depends(get_db)):
    ...
    main_cat = request.session.get("main_cat")
    sub_cat = request.session.get("sub_cat")
    name = request.session.get("name")
    brand = request.session.get("brand")
    price = request.session.get("price")
    stock = request.session.get("stock")
    image = request.session.get("image")
    detail_image = request.session.get("detail_image")
